...
import array, sys, dns.resolver, requests, json, math, time
...
def updateArtnet():
  ...
  for node in Nodes:
    statusmask = 0
    try:
      a = mDNS.resolve(node, 'A', 'IN', False, None, False, 0, 1) # who, what, class, tcp?, source ip, raise if no answer?, source port, lifetime
    except:
      a = None
    ...
    if response is not None:
      ...
      universe_amount = math.ceil(data.leds.count / MaxLedsInUniverse)
